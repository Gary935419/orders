		...
		<div class="layui-col-md12">
			<div class="layui-card">
				<div class="layui-card-body ">
					<form class="layui-form layui-col-space5" method="get" action="<?= RUN, '/member/userorder_list' ?>">
				...
				<div class="layui-card-body ">
					<table class="layui-table layui-form">
						...
						<tr>
							...
							<th style="width: 10%">订单费用</th>
							<th style="width: 10%">回收商家</th>
						</thead>
						<tbody>
						<?php if (isset($list) && !empty($list)) { ?>
							<?php foreach ($list as $num => $once): ?>
								<tr>
									<td><?= $num + 1 ?></td>
									<td><?PHP if($once['otype']==0){echo "自己送货";}else{echo "上门取货";}; ?></td>
									<td><?PHP if($once['ostate']==0){echo "已预约";}elseif($once['ostate']==1){echo "待回收";}elseif($once['ostate']==2){echo "已完成";}elseif($once['ostate']==3){echo "已取消";} ?></td>
									<td><?= date("Y-m-d",$once['addtime']); ?></td>
									<td><?= $once['uname'] ?></td>
									<td><?= substr($once['proname'],2) ?></td>
									<td><?= $once['moneys'] ?></td>
									<td><?php echo $once['merchantsname']; ?></td>
								</tr>
							<?php endforeach; ?>
						<?php } else { ?>
							<tr>
