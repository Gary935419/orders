		...
		<div class="layui-col-md12">
			<div class="layui-card">
				...
				<button class="layui-btn layui-card-header" style="float: right;margin-top: -40px;margin-right: 20px;"
						onclick="xadmin.open('添加','<?= RUN . '/promote/couple_add' ?>',900,500)"><i
				...
				<div class="layui-card-body ">
					<table class="layui-table layui-form">
						...
						<tr>
							...
							<th style="width: 20%">礼品名</th>
							<th style="width: 20%">礼品图片</th>
							<th style="width: 20%">礼品数量</th>
						...
						<?php if (isset($list) && !empty($list)) { ?>
							<?php foreach ($list as $num => $once): ?>
								<tr id="p<?= $once['cid'] ?>" sid="<?= $once['cid'] ?>">
									<td><?= $num + 1 ?></td>
									<td><?= $once['couple_goodsname'] ?></td>
									<td>
										<img class="layui-upload-img" src="<?php echo $once['couple_goodsimg'] ?>" style="width: 100px;height: 100px;"></td>
									<td><?= $once['couple_num'] ?></td>
									<td class="td-manage">
										<button class="layui-btn layui-btn-normal"
												onclick="xadmin.open('编辑','<?= RUN . '/promote/couple_edit?id=' ?>'+<?= $once['cid'] ?>,900,500)">
											<i class="layui-icon">&#xe642;</i>编辑
										</button>
										<button class="layui-btn layui-btn-danger"
												onclick="couple_delete('<?= $once['cid'] ?>')"><i class="layui-icon">&#xe640;</i>删除
										</button>
									</td>
								</tr>
							<?php endforeach; ?>
